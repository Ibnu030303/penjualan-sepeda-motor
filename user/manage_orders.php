...
$sql = "SELECT sale.sale_id, sale.sale_date, sale.total_price, sale.payment_type, customers.customer_id, customers.name AS customer_name, motorcycles.motorcycle_id, motorcycles.model AS motorcycle_model, motorcycles.brand AS motorcycle_brand 
        FROM sale 
        JOIN customers ON sale.customer_id = customers.customer_id 
        JOIN motorcycles ON sale.motorcycle_id = motorcycles.motorcycle_id";
...
$sql_motorcycles = "SELECT motorcycle_id, brand, model, price, stock FROM motorcycles";
...
<body>
                                            ...
                                            <tbody>
                                                ...
                                                <?php while ($row = $result->fetch_assoc()) : ?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo $row['motorcycle_brand'] . ' ' . $row['motorcycle_model']; ?></td>
                                                        <td><?php echo $row['sale_date']; ?></td>
                                                        <td class="total_price"><?php echo $row['total_price']; ?></td>
                                                        <td><?php echo $row['payment_type']; ?></td>
                                                        <td>
                                                            <button type="button" class="btn btn-warning btn-sm edit-btn" data-id="<?php echo $row['sale_id']; ?>" data-customer_id="<?php echo $row['customer_id']; ?>" data-motorcycle_id="<?php echo $row['motorcycle_id']; ?>" data-sale_date="<?php echo $row['sale_date']; ?>" data-total_price="<?php echo $row['total_price']; ?>" data-payment_type="<?php echo $row['payment_type']; ?>" data-bs-toggle="modal" data-bs-target="#SaleModal">Edit</button>
                            ...
                            <form method="POST" action="manage_orders.php" enctype="multipart/form-data">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="motorcycle_id" name="motorcycle_id" aria-label="Floating label select example" onchange="updatePrice()" required>
                                            <option value="" data-price="">Select a motorcycle</option>
                                            <?php while ($row = $result_motorcycles->fetch_assoc()) : ?>
                                                <option value="<?= $row['motorcycle_id'] ?>" data-price="<?= $row['price'] ?>"><?= $row['model'] ?></option>
                                            <?php endwhile; ?>
                                        </select>
                                    </div>
                                    <div class="mb-3">
    ...
    <script>
        function updatePrice() {
            var select = document.getElementById('motorcycle_id');
        ...
        $(document).ready(function() {
            ...
            $('.edit-btn').click(function() {
                ...
                var motorcycle_id = $(this).data('motorcycle_id');
                ...
                $('#motorcycle_id').val(motorcycle_id);
